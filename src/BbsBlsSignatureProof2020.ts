...
import {
  ...
  blsVerifyProof,
} from "@kskhasegawa/bbs-signatures";
import {
  DeriveProofOptions,
...
export class BbsBlsSignatureProof2020 extends suites.LinkedDataProof {
  ...
  async deriveProofWithOutput(options: DeriveProofOptions): Promise<object> {
    const {
      document,
      proof,
      revealDocument,
    ...
    // Compute the proof
    const output: ProofResult = await blsCreateProofWithOutput({
      signature: new Uint8Array(signature),
      publicKey: new Uint8Array(key.publicKeyBuffer),
      messages: allInputStatements,
      nonce: nonce,
